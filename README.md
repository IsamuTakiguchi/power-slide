...
- リボンの折りたたみ — 選んでいるタブをもう一度押すか、タブ列の右端のボタンで中身をたたむ
...
画面の幅を見て 3 段階に切り替わります（同じ画面・同じ機能のまま、置き場所だけを変えています）。

| 段階 | 幅 | 配置 |
| --- | --- | --- |
| PC | 901px 以上 | PowerPoint と同じ 2 段組みのリボン、左にスライド一覧、右に書式設定 |
| タブレット | 601〜900px | リボンはグループ名を省いた 1 行のボタン列（横スクロール）、スライド一覧は細い縦並び、書式設定とメニューは下から出るシート |
| スマホ | 600px 以下 | さらにリボンとタブを画面の下（親指の届く位置）に移し、スライド一覧は下の帯（横スクロール） |

タッチ操作向けの調整:

- ボタンは指で押せる大きさ（44px 前後）にし、メニューは下から出るシートにする
- テキストは 1 回タップで選択、もう一度タップで文字を直せる（マウスはダブルクリックのまま）
- スライド一覧の末尾の「追加」でスライドを増やせる
- スライドショーは画面のタップで送れる（左端 3 割で戻る）
- リボンは選んでいるタブをもう一度押すか、タブ列の右端のボタンでたためる（横持ちでスライドを広く見せたいとき）
- ソフトキーボードが出ている間は、その高さぶん画面を詰めて入力中の場所が隠れないようにする
- 端末の切り欠き・ステータスバー・ホームバーを避ける（ホーム画面に追加したアプリでは画面が
  ステータスバーの下まで広がるため、上端にもその高さぶんの余白を入れている）
- アドレスバーの出入りで下のバーが隠れないようにしている（`100dvh`）
...
要素はダブルクリックでその場編集、`Shift` + クリックで追加選択できます（タッチ操作では
選択済みの要素をもう一度タップするとその場編集に入ります）。
